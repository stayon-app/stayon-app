...
export function ProfileScreen({ navigation }: any) {
  ...
  const stats = [
    ...
    { label: listingCount === 1 ? 'Listing' : 'Listings', value: String(listingCount) },
  ];
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: spacing['4xl'] }}>
        ...
        <View style={styles.hero}>
          ...
          <TouchableOpacity style={styles.editBtn} activeOpacity={0.85} onPress={() => { light(); navigation.navigate('Listings'); }}>
            ...
            <Text style={styles.editText}>Manage your listings</Text>
          </TouchableOpacity>
        </View>

        <SettingsSection title="Hosting">
          <SettingsRow icon="home-outline" title="Your listings" subtitle="Create & manage properties" onPress={go('Listings')} />
        ...
        <SettingsSection title="Insights & operations">
          ...
          <SettingsRow icon="wallet-outline" title="Payouts" subtitle="Scheduled & paid history" onPress={go('Payouts')} />
        ...
        <TouchableOpacity style={styles.switchPill} activeOpacity={0.9} onPress={switchToGuest}>
...
const makeStyles = (colors: any) =>
  StyleSheet.create({
    ...
    switchPill: { flexDirection: 'row', alignSelf: 'center', alignItems: 'center', gap: 8, marginTop: spacing.xl, paddingHorizontal: spacing.xl, paddingVertical: spacing.md, borderRadius: borderRadius.full, backgroundColor: colors.textPrimary },
