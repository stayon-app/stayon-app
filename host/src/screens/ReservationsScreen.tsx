...
export function ReservationsScreen({ navigation }: any) {
  ...
  const accept = async (r: HostReservation) => { success(); setAll(await setReservationStatus(r.id, 'confirmed')); };
  const decline = (r: HostReservation) => confirmAction({
    ...
    onConfirm: async () => setAll(await setReservationStatus(r.id, 'cancelled')),
  });
