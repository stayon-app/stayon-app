...
export function ReservationDetailScreen({ navigation, route }: any) {
  ...
  const accept = async () => { success(); const all = await setReservationStatus(r.id, 'confirmed'); setR(all.find((x) => x.id === r.id) ?? null); };
  const decline = () => confirmAction({
    ...
    onConfirm: async () => { const all = await setReservationStatus(r.id, 'cancelled'); setR(all.find((x) => x.id === r.id) ?? null); },
  });
