...
// Security headers — sensible defaults for an API (CSP belongs on the web app).
...
// Simple in-memory rate limiter (per IP). NOTE: for multi-instance/global scale,
// swap this for a Redis-backed limiter so the window is shared across nodes.
...
// Periodically purge expired rate-limit records so the map can't grow unbounded.
...
// =========================================================================
app.get('/v1/me/export', authUser, wrap(async (req, res) => {
...
app.post('/v1/me/delete', authUser, wrap(async (req, res) => {
  ...
  await audit(req, 'gdpr.delete', 'user', uid);
  ok(res, { deleted: true });
}));
...
// Catch-all 404 + central error handler (defence in depth alongside per-route wrap()).
...
app.use((e, req, res, next) => { // eslint-disable-line no-unused-vars
  console.error(JSON.stringify({ t: new Date().toISOString(), level: 'error', path: req.path, msg: e?.message }));
...
function shutdown(sig) {
  ...
  setTimeout(() => process.exit(1), 10_000).unref(); // hard cap if a request hangs
}
process.on('SIGTERM', () => shutdown('SIGTERM'));
process.on('SIGINT', () => shutdown('SIGINT'));
// Never crash the whole service on an unhandled rejection — log and continue.
