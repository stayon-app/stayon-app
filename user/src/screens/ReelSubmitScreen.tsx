...
import { getMyReels, submitReel, deleteReel, STATUS_META, type Reel } from '../data/reels';
import { Api } from '../api';
...
const STEP_META = [
  { n: 'Step 1', t: 'Add your reel', s: 'Pick a video straight from your gallery — iPhone & Android high‑quality clips work great.' },
  { n: 'Step 2', t: 'Describe it', s: 'A title and location help others discover your reel.' },
  { n: 'Step 3', t: 'Review & submit', s: 'Check it over — the StayOn team reviews every reel before it goes live.' },
];

const GUIDELINES = [
  'Real footage of a stay or place — no stock or reposts',
  'Vertical video, 15–60 seconds, good lighting',
  'No faces of other guests without consent; no contact info',
  'Family‑friendly — no offensive content',
];

export const ReelSubmitScreen: React.FC = () => {
  ...
  useFocusEffect(React.useCallback(() => {
    let active = true;
    getMyReels().then((r) => { if (active) setMine(r); });
    return () => { active = false; };
  }, []));
  ...
  const next = async () => {
    ...
    setMine(updated.filter((r) => r.author === 'you'));
    // Backend: submit for Ops moderation (→ pending → live feed). Fail-safe.
    try {
  ...
  const remove = async (id: string) => { selection(); setMine((await deleteReel(id)).filter((r) => r.author === 'you')); };

  const meta = STEP_META[i];

  // Only signed-in StayOn users can post a reel.
  if (!isAuthenticated) {
    return (
      <SafeAreaView style={styles.container} edges={['top']}>
        <View style={styles.topBar}>
          <TouchableOpacity style={styles.topPill} onPress={() => navigation.goBack()} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <Ionicons name="close" size={20} color={colors.textPrimary} />
          </TouchableOpacity>
          <Text style={styles.topTitle}>Post a StayReel</Text>
          <View style={{ width: 36 }} />
        </View>
        <View style={styles.gate}>
          <View style={styles.gateIcon}><Ionicons name="lock-closed" size={30} color={colors.primary} /></View>
          <Text style={styles.gateTitle}>Log in to post a reel</Text>
          <Text style={styles.gateText}>Only signed-in StayOn members can share reels. Log in or sign up — it takes a few seconds.</Text>
          <TouchableOpacity activeOpacity={0.9} onPress={() => { light(); navigation.navigate('Auth', { returnAction: () => navigation.navigate('ReelSubmit') }); }}>
            <LinearGradient colors={STAYON_GRADIENT} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.gateBtn}>
              <Text style={styles.gateBtnText}>Log in or Sign up</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing['2xl'] }}>
        ...
        {justSubmitted && i === 0 && (
          <View style={styles.successBar}>
            ...
            <Text style={styles.successText}>Submitted! Our team reviews reels (usually 24–48h). Approved reels go live in StayReels.</Text>
          </View>
        )}
        ...
        {i === 0 && (
          <>
            <TouchableOpacity style={styles.upload} activeOpacity={0.85} onPress={pickVideo}>
              {videoUri ? (
                <>
                  ...
                  <View style={styles.playPill}><Ionicons name="checkmark-circle" size={18} color="#fff" /><Text style={styles.playPillText}>Video added</Text></View>
                ...
                <>
                  ...
                  <View style={styles.uploadInner}>
                    ...
                    <Text style={styles.uploadSub}>Pick a vertical video to share</Text>
            ...
            {mine.length > 0 && (
              <>
                <Text style={styles.section}>Your reels</Text>
                {mine.map((r) => {
        ...
        {i === 1 && (
          <>
            ...
            <TextInput style={styles.input} value={title} onChangeText={setTitle} placeholder="e.g. Sunset at the cliff villa" placeholderTextColor={colors.textTertiary} />
            ...
            <TextInput style={[styles.input, { minHeight: 90 }]} value={caption} onChangeText={setCaption} placeholder="Say something about this moment…" placeholderTextColor={colors.textTertiary} multiline textAlignVertical="top" />
          </>
        )}
          ...
          <>
            <View style={styles.previewCard}>
              <View style={styles.previewMedia}>
                ...
                <View style={styles.previewText}>
                  <Text style={styles.previewTitle} numberOfLines={1}>{title || 'Your reel'}</Text>
            ...
            <View style={styles.guidelines}>
              ...
              {GUIDELINES.map((g, idx) => (
                <View key={idx} style={styles.guideRow}><Ionicons name="checkmark" size={14} color={colors.primary} /><Text style={styles.guideText}>{g}</Text></View>
              ))}
              <Text style={styles.guideNote}>Every reel is reviewed by the StayOn team before it appears live — this keeps the feed real and high‑quality.</Text>
            </View>
          </>
        )}
      </ScrollView>
...
const makeStyles = (colors: any) =>
  StyleSheet.create({
    ...
    topPill: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.card, borderWidth: 1, borderColor: colors.borderLight },
    ...
    stepTitle: { fontSize: fontSizes['2xl'], color: colors.textPrimary, marginTop: 4, ...fonts.bold },
    ...
    successBar: { flexDirection: 'row', gap: 8, alignItems: 'flex-start', padding: spacing.base, borderRadius: borderRadius.md, backgroundColor: colors.primaryUltraLight, marginBottom: spacing.base },
    successText: { flex: 1, fontSize: fontSizes.sm, color: colors.textPrimary, lineHeight: 18, ...fonts.medium },
    upload: { height: 300, borderRadius: borderRadius.lg, overflow: 'hidden', backgroundColor: colors.card, borderWidth: 1.5, borderColor: colors.borderLight, borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center' },
    uploadInner: { alignItems: 'center', gap: 6, paddingHorizontal: spacing.lg },
    uploadIcon: { width: 60, height: 60, borderRadius: 30, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.card, marginBottom: 6, borderWidth: 1, borderColor: colors.borderLight },
    uploadScrim: { backgroundColor: colors.card, opacity: 0.8 },
    ...
    changeText: { color: '#111', fontSize: fontSizes.xs, ...fonts.bold },
    ...
    previewCard: { borderRadius: borderRadius.lg, overflow: 'hidden', backgroundColor: colors.card, borderWidth: 1, borderColor: colors.borderLight },
    ...
    guidelines: { marginTop: spacing.lg, padding: spacing.base, borderRadius: borderRadius.lg, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.borderLight },
    ...
    myReel: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, padding: spacing.sm, borderRadius: borderRadius.lg, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.borderLight, marginBottom: spacing.sm },
    ...
    gate: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: spacing.xl, gap: 10 },
    gateIcon: { width: 72, height: 72, borderRadius: 36, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.primaryUltraLight, marginBottom: spacing.sm },
    gateTitle: { fontSize: fontSizes.xl, color: colors.textPrimary, ...fonts.bold },
    gateText: { fontSize: fontSizes.base, color: colors.textSecondary, textAlign: 'center', lineHeight: 22, marginBottom: spacing.md, ...fonts.regular },
    gateBtn: { paddingHorizontal: spacing.xl, paddingVertical: spacing.base, borderRadius: borderRadius.lg, alignItems: 'center' },
    gateBtnText: { color: '#fff', fontSize: fontSizes.md, ...fonts.bold },
  });

export default ReelSubmitScreen;
