...
export function TodayScreen({ navigation }: any) {
  ...
  const TOOLS = [
    ...
    { icon: 'chatbubbles-outline', label: 'Inbox', route: 'InboxTab', tab: true },
    { icon: 'star-outline', label: 'Reviews', route: 'Reviews' },
    { icon: 'trending-up-outline', label: 'Trends', route: 'Trends' },
    { icon: 'bulb-outline', label: 'Suggestions', route: 'SmartSuggestions' },
    { icon: 'wallet-outline', label: 'Payouts', route: 'Payouts' },
    { icon: 'construct-outline', label: 'Maintenance', route: 'Maintenance' },
    { icon: 'shield-checkmark-outline', label: 'Safety', route: 'Safety' },
    { icon: 'book-outline', label: 'Resources', route: 'Resources' },
    { icon: 'sparkles-outline', label: 'Assistant', route: 'HostAssistant' },
    { icon: 'add-circle-outline', label: 'New listing', route: 'ListingCreate' },
  ];
  const goTool = (t: any) => { light(); t.tab ? navigation.navigate(t.route) : navigation.navigate(t.route); };

  const wave = now.getHours() < 12 ? '🌅' : now.getHours() < 17 ? '☀️' : '🌙';

  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      {/* Decorative background — soft colour blobs behind the header */}
      <View pointerEvents="none" style={styles.bgDecor}>
        <View style={[styles.blob, { top: -40, left: -30, backgroundColor: withOpacity(colors.primary, 0.16) }]} />
        <View style={[styles.blob, { top: -60, right: -40, backgroundColor: withOpacity('#6366F1', 0.14) }]} />
        <View style={[styles.blob, { top: 60, right: 60, width: 90, height: 90, backgroundColor: withOpacity('#EC4899', 0.08) }]} />
      </View>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: spacing['4xl'] }}>
        {/* Greeting */}
        <View style={styles.header}>
          <View style={{ flex: 1 }}>
            <Text style={styles.eyebrow}>{todayStr.toUpperCase()}  {wave}</Text>
        ...
        {heroListing ? (
          <TouchableOpacity style={styles.heroCard} activeOpacity={0.92} onPress={() => { light(); navigation.navigate('ListingDetails', { id: heroListing.id }); }}>
            ...
            <View style={styles.heroBottom}>
              <Text style={styles.heroTitle} numberOfLines={1}>{heroListing.title || 'Your stay'}</Text>
          ...
          <TouchableOpacity style={styles.heroCard} activeOpacity={0.92} onPress={() => { light(); navigation.navigate('ListingCreate'); }}>
            ...
            <View style={styles.heroBottom}>
              ...
              <View style={[styles.manageBtn, { alignSelf: 'flex-start', marginTop: spacing.sm }]}><Ionicons name="add" size={15} color="#fff" /><Text style={styles.manageText}>Start a stay</Text></View>
            </View>
          </TouchableOpacity>
        )}
        ...
        <View style={styles.glanceRow}>
          <Glance icon="bar-chart-outline" tint={colors.primary} value={`${occupancy}%`} label="Occupancy" colors={colors} />
          <Glance icon="calendar-outline" tint={colors.primary} value={String(upcoming)} label="Upcoming" colors={colors} />
          <Glance icon="time-outline" tint={colors.primary} value={String(pending)} label="Requests" colors={colors} />
        </View>
        ...
        <View style={styles.toolsGrid}>
          {TOOLS.map((t) => (
            <TouchableOpacity key={t.label} style={styles.tool} activeOpacity={0.85} onPress={() => goTool(t)}>
...
const Glance = ({ icon, tint, value, label, colors }: any) => {
  ...
  return (
    <View style={styles.glance}>
      <View style={[styles.glanceIcon, { backgroundColor: withOpacity(tint, 0.12) }]}><Ionicons name={icon} size={18} color={tint} /></View>
...
const makeStyles = (colors: any) =>
  StyleSheet.create({
    ...
    bgDecor: { position: 'absolute', top: 0, left: 0, right: 0, height: 260, overflow: 'hidden' },
    blob: { position: 'absolute', width: 160, height: 160, borderRadius: 100 },
    ...
    glanceIcon: { width: 36, height: 36, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
    ...
    toolIconWrap: { width: 50, height: 50, borderRadius: 25, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.primarySubtle },
    toolBadge: { position: 'absolute', top: -3, right: -3, minWidth: 17, height: 17, borderRadius: 9, paddingHorizontal: 4, backgroundColor: colors.error, borderWidth: 1.5, borderColor: colors.card, alignItems: 'center', justifyContent: 'center' },
    toolBadgeText: { color: '#fff', fontSize: 9, ...fonts.bold },
    toolLabel: { fontSize: 11, color: colors.textPrimary, ...fonts.semiBold },
