...
export async function addStory(input: { title: string; body: string; cover: string }): Promise<HostStory[]> {
  const all = await getStories();
  const next: HostStory[] = [
    { id: `st_${Date.now()}`, title: input.title.trim() || 'Untitled story', body: input.body.trim(), cover: input.cover, status: 'pending', createdAt: Date.now() },
    ...all,
  ];
