...
/** Bridge: a review a GUEST wrote in the user app lands in the host's Reviews
 * tab. Newest first. */
export async function addGuestReview(r: { guestName: string; listingTitle: string; rating: number; text: string }): Promise<GuestReview[]> {
  const all = await getGuestReviews();
  const date = new Date().toLocaleString('en-US', { month: 'short', year: 'numeric' });
  const review: GuestReview = {
    id: `g_${Date.now()}`,
    guestName: r.guestName || 'Guest',
    guestAvatar: av(r.guestName || 'Guest'),
    listingTitle: r.listingTitle || 'your stay',
    rating: r.rating,
    text: r.text,
    date,
  };
